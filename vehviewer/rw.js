...
var rwID_FRAMELIST     = 0x0E; // 14
var rwID_GEOMETRY      = 0x0F; // 15
var rwID_CLUMP         = 0x10; // 16
var rwID_LIGHT         = 0x12; // 18
var rwID_ATOMIC        = 0x14; // 20
var rwID_RASTER        = 0x15; // 21
var rwID_TexDictionary = 0x16; // 21
var rwID_GEOMETRYLIST  = 0x1A; // 26
// tk
...
function RwCameraSetFrame(c, f)
{
	c.frame = f;
	f.objects.push(c);
}

function RwCameraBeginUpdate(cam)
{
...
function RwTextureRead(name, mask)
{
	if (name == "") return {
		name: name,
		mask: mask,
	...
	let fromTxd = myTxd?.content[name];
	if (!fromTxd) fromTxd = globalTxd?.content[name];
	// console.log(myTxd);
	if (fromTxd) {
		// console.log("TXD ", name);
		return {
			name: name,
			mask: fromTxd.mask,
			tex: fromTxd.tex,
	...
	console.log("TXD isn't included:", name);
	return {
		name: name,
		mask: mask,
...
function RpGeometryCreate(flags, numMorphTargets)
{
	let geo = {
...
function RpAtomicSetFrame(a, f)
{
	a.frame = f;
	f.objects.push(a);
}

function RpAtomicRender(atomic)
{
...
function instV3d(buffer, offset, stride, src, n)
{
...
function instV2d(buffer, offset, stride, src, n)
{
	let view = new DataView(buffer);
	let o = offset;
	for(let i = 0; i < n; i++){
...
function instRGBA(buffer, offset, stride, src, n)
{
	let view = new DataView(buffer);
	let o = offset;
	for(let i = 0; i < n; i++){
